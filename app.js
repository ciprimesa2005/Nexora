...
function handleSubscribe(inputId) {
  ...
  let subscriptions = JSON.parse(localStorage.getItem('aureon_subscriptions') || '[]');
  if (!subscriptions.includes(email)) {
    subscriptions.push(email);
    localStorage.setItem('aureon_subscriptions', JSON.stringify(subscriptions));
  }
...
function handleFormSubmit() {
  ...
  let tickets = JSON.parse(localStorage.getItem('aureon_support_tickets') || '[]');
  tickets.push({ name, email, message, date: new Date().toISOString() });
  localStorage.setItem('aureon_support_tickets', JSON.stringify(tickets));
...
function handleLogin() {
  ...
  localStorage.setItem('aureon_user_session', JSON.stringify({ email, loginTime: new Date().toISOString() }));
...
function handleChatReply(replyType) {
  ...
  // Define reply texts
  const userTexts = {
    planes: "Quiero saber más sobre los planes de Aureon.",
  ...
  setTimeout(() => {
    ...
    if (replyType === 'planes') {
      ...
      botResponse = `Si tienes una consulta compleja o necesitas soporte extra inmediato, presiona el botón a continuación para hablar directamente con un especialista en nuestro canal de WhatsApp de Soporte:<br>
        <a href="https://wa.me/573000000000?text=Hola%20Aureon,%20necesito%20asesoria%20digital" target="_blank" class="chat-whatsapp-btn">
