// Nexora — Cuestionario de Madurez Digital
// Storage: localStorage key "nexora_encuestas" (array of response records)

const STORAGE_KEY = 'nexora_encuestas';
...
function initSurveyForm() {
  ...
  form.addEventListener('submit', function (e) {
    ...
    saveResponse(record);
    sessionStorage.setItem('nexora_last_result', String(record.id));
...
function renderLastResultBanner(data) {
  ...
  if (!el) return;
  const lastId = sessionStorage.getItem('nexora_last_result');
  if (!lastId) return;
  ...
  el.innerHTML = `
    <div class="result-banner">
      ...
      <p class="rec">${nivel.desc} Nexora recomienda cruzar este resultado con el obstáculo señalado (“${record.obstaculo}”) para priorizar el plan de acción.</p>
    </div>
  `;
}
